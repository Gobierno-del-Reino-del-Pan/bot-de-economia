...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('denegarwork')
    .setDescription('Deniega comandos de trabajo en un canal específico (Solo administradores)')
    .addChannelOption(option =>
      option
        .setName('canal')
        .setDescription('Canal donde denegar los comandos de trabajo (opcional, por defecto canal actual)')
        .setRequired(false)
    ),
  adminOnly: true,
    ...
    try {
      ...
      if (!permitData.workChannels.includes(channel.id)) {
        const embed = new EmbedBuilder()
          ...
          .setDescription(`El canal ${channel} no está en la lista de canales permitidos para trabajo.`)
          .addFields({
            name: '💡 Información',
      ...
      permitData.workChannels = permitData.workChannels.filter(id => id !== channel.id);
      this.savePermitData(permitData);

      const embed = new EmbedBuilder()
        .setColor('#ff0000')
        .setTitle('❌ Canal de Trabajo Denegado')
        .setDescription(`El canal ${channel} ya no permite comandos de trabajo.`)
        .addFields(
          { name: '📝 Canal', value: `${channel}`, inline: true },
          { name: '🚫 Comandos Bloqueados', value: '`!trabajar`, `!work`, `!currar`', inline: true },
          { name: '👮 Administrador', value: interaction.user.username, inline: true }
        )
        .setFooter({ text: 'Los comandos de trabajo no funcionarán en este canal' })
        .setTimestamp();

      await interaction.reply({ embeds: [embed] });

    } catch (error) {
      console.error('Error en denegarwork:', error);
      const embed = new EmbedBuilder()
        .setColor('#ff0000')
